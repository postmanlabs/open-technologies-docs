    ...
    {links.map((item) => {
      ...
      if (item.type === "dynamic_blog" && item.blog_tag) {
        if (Object.keys(recentBlogPosts).length === 0) {
          ...
          return (
            <div className="contextual-links__alert" role="alert">
              <p>
                You are currently in develop mode. Dynamic blog posts will not
                be displayed locally.
                <a
                  style={{ fontSize: "inherit" }}
                  href="https://github.com/postmanlabs/open-technologies-docs/blob/develop/CONTRIBUTING.md#to-use-the-dynamic-blog-posts-feature"
                  target="_blank"
                  rel="noopener"
                >
        ...
        if (recentBlogPosts.index && recentBlogPosts.index[item.blog_tag]) {
          const dynamicLinks = recentBlogPosts.index[item.blog_tag]
            .slice(0, 3)
            .map((post) => (
              <div className="contextual-links__link" key={uuidv4()}>
      ...
      return (
        <div className="contextual-links__section" key={uuidv4()}>
          {item.name}
        </div>
      )
    })}
