...
const renderTwoLevelList = (item, runtime) => {
  if (typeof document !== "undefined") {
    const active = runtime
      ? document.location.pathname.match(item.parentSlug)
      : ""
    return (
          ...
          <div className="container">
            <div className="row">
              <CaretWrapper className="caret-wrapper" href={item.url}>
                {runtime && (
                  <svg
                    className={`caret${active ? " active-caret" : ""}`}
                ...
                (sItem) =>
                  (sItem.url && (
                    <li
                      key={uuidv4()}
                      className={`child ${
                        window.location.pathname === sItem.url
                          ? "currentUrl"
                          : ""
                      }`}
                    >
                      <Link data-click={sItem.name} to={sItem.url}>
                        {sItem.name}
                      </Link>
                    </li>
                  )) || (
                    <li key={uuidv4()} className="parent sub-parent">
                      <div className="container">
                        <div className="row">
                          <CaretWrapper
                            ...
                            (ssItem) =>
                              ssItem.url && (
                                <li
                                  key={uuidv4()}
                                  className={`child ${
                                    document.location.pathname === ssItem.url
                                      ? "currentUrl"
                                      : ""
                                  }`}
                                >
...
function LeftNav(props) {
  ...
  return leftNavItems.map((item) => renderTwoLevelList(item, runtime))
}
