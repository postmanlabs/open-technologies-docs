...
function PreviousAndNextLinks() {
  ...
  if (parentLinks.length > 1) {
    ...
    subParentLinks[subParentLinks.length + -1] === undefined
      ? subParentLinks.pop()
      : subParentLinks
  ...
  function handleSubMenu(links) {
    for (let i = 0; i < links.length; i++) {
      if (links[i].url === location) {
        ...
        previous =
          prevIndex && prevIndex.subMenuItems2
            ? prevIndex.subMenuItems2[prevIndex.subMenuItems2.length + -1]
            : prevIndex
  ...
  return (
    <>
      <hr />
      <div
        id="previousNextLinks"
        className="d-flex flex-row mt-3 pagination"
        role="navigation"
      >
        {prevLink && (
